...
// If MAPSVG_DEBUG_ROOT == true then MapSVG loads full (not minified) versions of JS/CSS files for debugging
define('MAPSVG_DEBUG_ROOT', false );

// If MAPSVG_RAND == true && MAPSVG_DEBUG == true then a random number is added to js/css file URLs to disable cache
define('MAPSVG_RAND', true );

// Last chance to turn on MAPSVG_DEBUG mode is pass a ?debug=1 parameter in the URL:
define('MAPSVG_DEBUG', (isset($_GET['debug']) ? (bool)$_GET['debug'] : MAPSVG_DEBUG_ROOT));
...
define('MAPSVG_INCOMPATIBLE_VERSIONS',  '3.2.0 5.0.0');
...
if(!empty($mapsvg_purchase_code)){
	require MAPSVG_PLUGIN_DIR.DIRECTORY_SEPARATOR.'plugin-update-checker/plugin-update-checker.php';
...
function mapsvg_print( $atts ){
    ...
    if($options && is_array($options)){
	    $options['data_regions'] = mapsvg_data_get_all(array(
		    ...
		    'with_schema' => true,
		    'perpage' => 0,
		    'sortBy' => ( $options['menu']['source'] == 'regions' ? $options['menu']['sortBy'] : ( strpos($options['menu']['source'],'geo-cal') !== false ? 'title' : 'id' ) ),
		    'sortDir' => $options['menu']['source'] == 'regions' ? $options['menu']['sortDirection'] : 'asc'
	    ));

	    $perpage = (int)$options['database']['pagination']['on'] ? $options['database']['pagination']['perpage'] : 0;
	    ...
	    $options['data_db'] = mapsvg_data_get_all(array(
		    ...
		    'with_schema' => true,
		    'perpage' => $perpage,
		    'sortBy' => $options['menu']['source'] == 'database' ? $options['menu']['sortBy'] : 'id',
		    'sortDir' => $options['menu']['source'] == 'database' ? $options['menu']['sortDirection'] : 'desc'
	    ));
    ...
    $container_id = mapsvg_generate_container_id($post->ID, 0);
...
//	$status_field = $wpdb->get_row('SHOW COLUMNS FROM '.mapsvg_table_name($map_id, 'regions').' LIKE \'status\'');
    ...
    if($region_titles != $t_compare || ($region_ids != $r_compare)){
        if(!empty($regions)){
            ...
            if(MAPSVG_DEBUG_ROOT && $wpdb->last_error){
                \PC::debug($wpdb->last_error);
            }
        }
    }
                    ...
                    if(!empty($location)){
                        ...
                        if($addressYesCoordsNo || $addressNoCoordsYes){
                            ...
                            if($addressNoCoordsYes){
                                $response = mapsvg_geocoding($location['lat'].','.$location['lng']);
...
//        $wpdb->update(mapsvg_table_name($_POST['map_id'], $table), mapsvg_encode_data($_POST['map_id'], $table, $data), array('id'=>esc_sql($data['id'])));
		...
		if(MAPSVG_DEBUG_ROOT){
			\PC::debug('UPDATE '.mapsvg_table_name($_POST['map_id'], $table).' SET '.implode(',',$set).' WHERE id IN (\''.implode('\',\'',$id).'\')');
		}
	}
	else{
...
function mapsvg_data_get_all($query){
    ...
    if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
        ...
        $t = $wpdb->get_var("SHOW TABLES LIKE '$table_name'");
	    $response['error'] = $t.' != '.$table_name;
...
//                $searchable_fields = explode(',',$searchable_fields);
                ...
                foreach($searchable_fields as $index=>$f){
                    if((isset($f['type']) && $f['type'] == 'region') || (isset($f['multiselect'])&&$f['multiselect']===true)){
                        ...
                        $match[] = ($f['type'] === 'location' ? $f['name'].'_address' : $f['name']);
                    }
                }
                if(count($match))
...
function mapsvg_set_db($map_id, $table, $schema, $prev_schema) {
    ...
    foreach($schema as $field){
        ...
        if($field['type'] == 'location'){
            $fields[] = 'location_lat FLOAT';
            $fields[] = 'location_lng FLOAT';
        ...
        if(isset($field['searchable']) && $field['searchable'] == 'true')
            if(($field['type']=='text' || $field['type']=='textarea' || $field['type']=='region')) {
                ...
                $searchable_fields[] = $field['name'].'_address';
            } else {
