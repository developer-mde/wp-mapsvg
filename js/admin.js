...
(function( $ ) {
    ...
    methods = {
        ...
        resizeSVGCanvas : function(){
            ...
            var h  = msvg.getData().$header.is(':hidden') ? 0 : Math.floor(msvg.getData().$header.outerHeight(true));
        ...
        setEventHandlersMainScreen : function(){
            ...
            $('#mapsvg-admin').on("click", "#mapsvg-btn-activate", function(e){
                ...
                $.post(ajaxurl, {purchase_code: code, action: "mapsvg_save_purchase_code"}, function(data){
                    data = JSON.parse(data);
                    if(data.error_message){
                        alert(data.error_message);
                    } else if (data && data.ok){
                        $("#mapsvg-alert-activate").hide();
                        alert('MapSVG is activated. You will receive updates notifications on the "Plugins" page and ' +
            ...
            $('.mapsvg-copy-shortcode').on('click',function(){
                ...
                $.growl.notice({title: "", message: "Shortcode copied to clipboard", duration: 700});
            });

            $(".select-map-list").mselect2().on("select2:select",function(){
            ...
            $('#svg_file_uploader').on('change',function(event){
                $.each(event.target.files, function(index, file) {
                    ...
                    reader.onload = function(event) {
                        ...
                        var postMaxSizeInMb = mapsvg_ini_vars.post_max_size.indexOf("M") ? mapsvg_ini_vars.post_max_size.replace("M",'') :  mapsvg_ini_vars.post_max_size/(1024*1024);
                        var fileSizeInMbString = fileSizeInMb.toFixed(2)+'Mb';
                        if(fileSizeInMb > postMaxSizeInMb){
                            $.growl.error({title: "Error", message: 'Can\'t upload the file. File size ('+fileSizeInMbString+') exceeds php.ini "post_max_size" value ('+mapsvg_ini_vars.post_max_size+'). More info: <a href="https://mapsvg.com/docs/troubleshooting/uploading-big-files" target="_blank">Uploading big files</a>', duration: 50000});
                            btn._button('reset');
                            return false;
                        }
            ...
            $("#svg_file_uploader_form").on('submit', function(form) {
                ...
                $.each(files, function(index, file) {
                    var filename = file.filename.replace(/\s/g, "_");
                    $.ajax({
                        url: ajaxurl,
                        type: 'POST',
                        data: {action: 'mapsvg_upload', filename: filename, data: file.data}
                    }).done(function(filename, status, xhr) {
                        $.growl.notice({title: "", message: 'File uploaded:<br>user-uploads/'+filename});
            ...
            $("#image_file_uploader_form").on('submit', function(form) {
                ...
                $.each(imgfiles, function(index, file) {
                    var filename = file.filename.replace(/\s/g, "_");
                    var fileSizeInMb = file.data.length/(1024*1024);
                    var postMaxSizeInMb = mapsvg_ini_vars.post_max_size.indexOf("M") ? mapsvg_ini_vars.post_max_size.replace("M",'') :  mapsvg_ini_vars.post_max_size/(1024*1024);
                    var fileSizeInMbString = fileSizeInMb.toFixed(2)+'Mb';
                    if(fileSizeInMb > postMaxSizeInMb){
                        $.growl.error({title: "Error", message: 'Can\'t upload the file. File size ('+fileSizeInMbString+') exceeds php.ini "post_max_size" value ('+mapsvg_ini_vars.post_max_size+'). More info: <a href="https://mapsvg.com/docs/troubleshooting/uploading-big-files" target="_blank">Uploading big files</a>', duration: 50000});
                    ...
                    $.ajax({
                        ...
                        window.location.href = '?page=mapsvg-config&map_id=new&package=uploads&path='+filename+'&noheader=true';
                    }).always(function(){
                        btn._button('reset');
                    });
                });
        ...
        setEventHandlers : function(){
            ...
            $('#mapsvg-admin').on('mousewheel','.jspContainer',
                    function(e) {
                        e.preventDefault();
                    }
                );
        ...
        showGoogleMapDownloader: function(){
            ...
            if(!_this.gmloaded){
                ...
                _data.googleMapsScript.src = 'https://maps.googleapis.com/maps/api/js?key='+_this.gmapikey+'&language=en';//+'&callback=initMap';
