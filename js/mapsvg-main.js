...
MapSVG.DatabaseService.prototype.import = function(data){
    ...
    return this.server.import(data, language).done(function(){
        _this.getAll();
    });
};
...
MapSVG.DatabaseService.prototype.getAll = function(params){
    ...
    var data = {
        ...
        perpage: _this.query.perpage !== undefined ? parseInt(_this.query.perpage) : this.perpage
    };
...
MapSVG.ServerMySQL.prototype.import = function(data, language){
    var _this = this;
    ...
    if(chunks.length > 0){
        ...
        chunks.forEach(function(chunk){
            ...
            setTimeout(function(){
                ...
                $.post(ajaxurl, _this.actionData('mapsvg_data_import', {data: _data, language: language}), null, 'json').done(function(_data){
...
MapSVG.DirectoryController.prototype.selectItems = function(ids){
    ...
    _this.scrollable && _this.contentWrap.nanoScroller({scrollTo: $('#mapsvg-directory-item-'+ids[0])});
};
...
MapSVG.Map.prototype = {
    ...
    setTemplates: function(templates){
        ...
        for (var name in templates){
            if(name != undefined){
                ...
                if(_this.editMode && (name == 'directory' && _this.controllers && _this.controllers.directory)){
    ...
    setViewBox : function(v,skipAdjustments){
        ...
        _this.scroll.tx = (_this.svgDefault.viewBox[0]-_this.viewBox[0])*_this.scale;
        _this.scroll.ty = (_this.svgDefault.viewBox[1]-_this.viewBox[1])*_this.scale;
    ...
    redraw: function(){
        ...
        if(MapSVG.browser.ie){
            _this.whRatio2 = _this.svgDefault.viewBox[2] / _this.svgDefault.viewBox[3];
            // MapSVG.get(0).getData().svgDefault.viewBox[2] / MapSVG.get(0).getData().svgDefault.viewBox[3];
            _this.$svg.height(_this.$map.outerWidth() / _this.whRatio2);
        }
    ...
    setControls : function (options){
        ...
        if(!_this.$controls){
            ...
            zoomIn.on(event,function(e){
                e.stopPropagation();
                _this.zoomIn();
            });

            var zoomOut = $('<div />').addClass('mapsvg-btn-map mapsvg-out');
            zoomOut.on(event,function(e){
                e.stopPropagation();
                _this.zoomOut();
            });
            ...
            zoomResetButton.on(event,function(e){
                e.stopPropagation();
    ...
    setMenu : function(options){
        ...
        if(_this.options.menu.on){
            ...
            _this.setFilters();
            if((_this.options.menu.source === 'regions') || (_this.options.menu.source === 'database' && _this.database.loaded)){
    ...
    loadGoogleMapsAPI : function(callback, fail){
        ...
        _this.googleMapsScript.src = 'https://maps.googleapis.com/maps/api/js?language=en&key='+_this.options.googleMaps.apiKey+drawing;
                        ...
                        events: {
                            load: function(_formBuilder){
                                ...
                                if(_this.options.filters.clearButton){
                                    var clearButton = $('<div class="form-group" style="flex-grow:0;"><div class="col-sm-3"></div><div class="col-sm-9"><button class="btn btn-default mapsvg-filters-reset">'+_this.options.filters.clearButtonText+'</button></div></div>');
    ...
    getUserLocation : function(callback){
        ...
        callback({lat: 29.7604, lng: -95.3698});
        return;
    ...
    centerOn : function(region, yShift){
        ...
        if(_this.options.googleMaps.on){
            ...
            _this.googleMaps.map.setCenter(latLng);
            setTimeout(function(){
    ...
    scrollMove :  function (e){
        ...
        if(_this.panBy((_this.scroll.gx - ce.clientX),(_this.scroll.gy - ce.clientY))){
            if(_this.googleMaps.map){
                // var coords = _this.getCenter();
                // _this.googleMaps.map.setCenter(coords);
                // _this.googleMaps.map.panBy((_this.scroll.gx - ce.clientX),(_this.scroll.gy - ce.clientY));
                var point = _this.googleMaps.map.getCenter();

                // var overlay = new google.maps.OverlayView();
                // overlay.draw = function() {};
                // overlay.setMap(_this.googleMaps.map);

                var projection = _this.googleMaps.overlay.getProjection();

                var pixelpoint = projection.fromLatLngToDivPixel(point);
                pixelpoint.x += _this.scroll.gx - ce.clientX;
                pixelpoint.y += _this.scroll.gy - ce.clientY;
    ...
    panBy : function(x, y){
        ...
        if(!_this.options.googleMaps.on && _this.options.scroll.limit){
    ...
    selectRegion :    function(id, skipDirectorySelection){
        ...
        region.select();
        if(!skipDirectorySelection && _this.options.menu.on && _this.controllers && _this.controllers.directory){
            if(_this.options.menu.source == 'database'){
                if(region.objects && region.objects.length) {
                    var ids = region.objects.map(function(obj){
                        return obj.id;
    ...
    convertSVGToPixel : function(xy){
        ...
        return [(xy[0]-_this.svgDefault.viewBox[0])*scale, (xy[1]-_this.svgDefault.viewBox[1])*scale];
    },
    /**
    ...
    setEventHandlers : function(){
        ...
        if(_this.editMarkers.on){
            ...
            _this.$map.on(event2, '.mapsvg-marker',function(e){
        ...
        if(_this.options.scroll.spacebar){
            ...
            if(!_this.editMarkers.on) {
                ...
                //         _this.touchScrollStart = $('body').scrollTop();
                //     });
                //     _this.$map.on('touchend.common.mapsvg', '.mapsvg-region', function (e) {
                //         if(_this.touchScrollStart == $('body').scrollTop()){
                ...
                //         _this.touchScrollStart = $('body').scrollTop();
                //     });
                //     _this.$map.on('touchend.common.mapsvg', '.mapsvg-marker', function (e) {
                //         if(_this.touchScrollStart == $('body').scrollTop()){
    ...
    init: function(opts, elem) {
        ...
        $.ajax({url: _this.options.source+'?v='+_this.options.svgFileVersion}).fail(function(resp){
            ...
            if(!_this.options.data_regions || !_this.options.data_db){
                ...
                if(_this.options.database.loadOnStart){
...
MapSVG.Marker.prototype.adjustPosition = function(){
    ...
    if(this.textLabel){
        this.textLabel[0].style.transform = 'translate(-50%, 0%) translate(' + (pos[0]+this.width/2-this.textLabel.outerWidth()/2) + 'px,' + (pos[1] - (this.height||23)) + 'px)';
    }
};
/**
